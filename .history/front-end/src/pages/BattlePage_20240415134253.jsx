...
const BattlePage = () => {
    ...
    const [currentPokemon, setCurrentPokemon] = useState(null);  // Updated to be null initially
    const [cynthiaPokemon, setCynthiaPokemon] = useState([]);
    ...
    // Function to set the current Pokemon
    const selectPokemon = (pokemon) => {
        setCurrentPokemon(pokemon);

        const randomIndex = Math.floor(Math.random() * cynthiaTeam.length);
        const randomCynthiaPokemon = cynthiaTeam[randomIndex];
        setCynthiaPokemon(randomCynthiaPokemon);
    };
                    ...
                    {ourTeam.map(pokemon => (
                        <Card key={pokemon.pokemon.name} className="mb-2">
                            <Card.Body>
                                ...
                                <Button variant="primary" onClick={() => selectPokemon(pokemon)}>Select</Button>
                            </Card.Body>
                        </Card>
                    ))}
                </Col>
