...
const BattlePage = () => {
    ...
    const handleAttack = (userMove) => {
        ...
        if (!userHasAttacked) {
            ...
            if (currentPokemon.pokemon.hp <= 0) {
                alert("Your current Pokémon has fainted! Please select another Pokémon.")
                return null; // Stop the function if the user's Pokémon has fainted
            }
            ...
            if (cynthiaPokemon.pokemon.hp <= 0) {
                ...
                if (nextPokemon) {
                    ...
                    // alert("Cynthia has no more Pokémon left! You win!")
                    return null; // Stop the function if no Pokémon are left for Cynthia
                }
            }
    ...
    const createBattleEvent = async (attacker, move, damage, effectivenessMessage) => {
        try {
            const response = await axios.post('http://localhost:8000/api/teams/battle_commentary/', {
                text: `${attacker.pokemon.name} uses ${move.learnable_move.name} and deals ${damage} damage! ${effectivenessMessage}`
            });
    
            setEvents(prevEvents => [...prevEvents, response.data.commentary]);  // Use the commentary from the backend
        } catch (error) {
    ...
    // const performBattle = async (firstAttacker, secondAttacker, firstMove, secondMove) => { // open AI func
    //     // Execute the first attack
    //     const newHPAfterFirstAttack = executeAttack(firstAttacker, secondAttacker, firstMove);
    ...
    //     setTimeout(async () => {
    //         if (newHPAfterFirstAttack > 0) {
    //             // Determine who should act next based on the previous attack result
    //             if (secondAttacker === cynthiaPokemon) {
    //                 // Fetch OpenAI's decision for Cynthia's move when it's her turn
    //                 const decision = await fetchCynthiasDecision(secondAttacker);
    //                 handleCynthiaDecision(decision);
    //             } else {
    //                 // Execute the planned move for the other attacker (usually the player)
    //                 executeAttack(secondAttacker, firstAttacker, secondMove);
    //             }
    //         } else {
    //             // Handle the situation where the current Pokémon has fainted
    //             if (secondAttacker === cynthiaPokemon) {
    //                 const nextPokemon = await fetchNextOpenAICynthiaPokemon();
    //                 if (nextPokemon) {
    //                     setCynthiaPokemon(nextPokemon);
    //                     setEvents(prevEvents => [...prevEvents, "Cynthia switched to " + nextPokemon.pokemon.name]);
    //                 } else {
    //                     alert("Cynthia has no more Pokémon left! You win!");
    //                 }
    //             }
    ...
    // const fetchCynthiasDecision = async (pokemon) => {
    //     try {
    //         const response = await axios.post('http://localhost:8000/api/teams/cynthia_decision/', {
    //             pokemon: pokemon.pokemon.pokedex_number,
    //             ourTeam: ourTeam.map(p => p.pokemon.pokedex_number),
    //             ourTeamHP: ourTeamHP,
    //             cynthiaTeamHP: cynthiaTeamHP
    //         });
    
    //         console.log("Decision received:", response.data);
    //         return response.data;
    //     } catch (error) {
    //         console.error("Error fetching decision:", error.response || error);
    //         throw error;
    //     }
    // };
    ...
    // const fetchNextOpenAICynthiaPokemon = async () => {
    //     try {
    //         const response = await axios.get('http://localhost:8000/api/teams/cynthia_next_pokemon/');
    //         if (response.data && response.data.pokemon) {
    //             console.log("Next Pokemon:", response.data.pokemon);
    //             return response.data.pokemon;  // Assuming the backend returns the pokemon directly
    //         } else {
    //             throw new Error("No Pokémon returned from server");
    //         }
    //     } catch (error) {
    ...
    // const handleCynthiaDecision = (decision) => {
    //     if (decision && decision.dialogue) {
    //         setEvents(prevEvents => [...prevEvents, decision.dialogue]);
    //         executeAttack(cynthiaPokemon, currentPokemon, decision.move);
    //     } else {
