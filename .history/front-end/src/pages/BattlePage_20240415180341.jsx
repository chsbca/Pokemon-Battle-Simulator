...
const BattlePage = () => {
    ...
    // State to track the current attack details
    const [attackDetails, setAttackDetails] = useState(null);

    useEffect(() => {
        if (attackDetails) {
            const { attacker, defender, damage } = attackDetails;
            setEvents(prevEvents => [`${attacker.pokemon.name} uses ${attackDetails.moveName} and deals ${damage} damage!`, ...prevEvents]);

            if (attackDetails.newHP <= 0) {
            ...
            // Reset attack details to avoid re-running the effect unintentionally
    ...
    const executeAttack = (attacker, defender, move) => {
        ...
        // Prepare attack details for useEffect processing
        setAttackDetails({
            attacker,
            defender,
            damage,
            moveName: move.learnable_move.name,
            newHP
        });
    };
