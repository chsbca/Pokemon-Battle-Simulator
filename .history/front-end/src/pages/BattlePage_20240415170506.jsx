    ...
    const performBattle = (attacker, defender, move) => {
        ...
        if (newHP <= 0) {
            ...
            if (attacker === currentPokemon) {
                ...
                // Reset attack flags when user's Pokémon faints to allow immediate action upon switching
                setUserHasAttacked(false);
                setCynHasAttacked(false);
            }
        } else {
            if (userHasAttacked && cynHasAttacked) {
                resetBattleRound(); // Reset flags for the next round if both sides have acted
            } else {
                // Schedule the counterattack
