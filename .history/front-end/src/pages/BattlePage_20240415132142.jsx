...
const BattlePage = () => {
    ...
    useEffect(() => {
        ...
        const fetchTeam = async () => {
            try {
                const response = await axios.get('http://localhost:8000/api/battle/team'); // Adjust the URL as needed
                setTeam(response.data.pokemons); // Assume response data is structured correctly
                setCurrentBattlePokemon(response.data.pokemons[0]); // Set the first Pokémon as the current battle Pokémon
            } catch (error) {
