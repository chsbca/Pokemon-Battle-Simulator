...
export default function ProfilePage() {
    ...
    const fetchLearnableMoves = async (pokedexNumber, pokemonName) => {
        try {
            const token = localStorage.getItem('token');
            ...
            if (response.data && response.data.length) {
                const mappedMoves = response.data.map(move => ({
                    learnableMoveId: move.id,  // This should be the LearnableMoves ID
                    moveId: move.move_id,      // This is the actual Move ID
                    name: move.name,
                    description: move.description
                }));
                setLearnableMoves(mappedMoves);
            ...
            setSelectedPokemon({ id: response.data.teamPokemonId, pokedexNumber, name: pokemonName });
            setShowOffcanvas(true);
        } catch (error) {
            console.error('Failed to fetch learnable moves:', error);
            setLearnableMoves([]);
    ...
    const addChosenMove = async (learnableMoveId) => {
        const move = learnableMoves.find(m => m.learnableMoveId === learnableMoveId);
        if (!move) {
            console.error('Move not found in learnableMoves:', learnableMoveId);
            return;
        }
        try {
            const token = localStorage.getItem('token');
            const response = await axios.post(`http://localhost:8000/api/team_pokemon/${selectedPokemon.id}/chosen_moves/`, {
                move_id: learnableMoveId  // This is the LearnableMoves ID
            }, {
                headers: { Authorization: `Token ${token}` }
            });
            console.log(`Move '${move.name}' added successfully:`, response.data);
