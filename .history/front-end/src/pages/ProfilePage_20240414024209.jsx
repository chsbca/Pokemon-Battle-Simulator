...
export default function ProfilePage() {
    ...
    const fetchLearnableMoves = async (pokemonId, pokemonName) => {
        try {
            ...
            setLearnableMoves(response.data.map(move => ({
                id: move.name,  // This is a temporary fix
                name: move.name,
    ...
    //             headers: { Authorization: `Token ${token}` }
    //         });
    //         console.log('Learnable moves fetched:', response.data); // Inspect what you fetched
    ...
    }, [selectedPokemon]); // Only re-run the effect if `selectedPokemon` changes
                ...
                <Accordion>
                    {team.length > 0 ? team.map((pokemon, index) => (
                        <Accordion.Item eventKey={index.toString()} key={index}>
                            ...
                            <Accordion.Body>
                                <button className="btn btn-info mb-3" onClick={() => {console.log(`Fetching learnable moves for Pokémon ID: ${pokemon.id} with Pokédex Number: ${pokemon.pokedex_number}`), fetchLearnableMoves(pokemon.pokedex_number, pokemon.id)}}>
                ...
                <Offcanvas.Body>
                    {learnableMoves.length ? (
                        learnableMoves.map((move, idx) => (
                            ...
                            <p key={idx}>
                                <button
                                    onClick={() => addChosenMove(move.id, selectedPokemon.id)}
