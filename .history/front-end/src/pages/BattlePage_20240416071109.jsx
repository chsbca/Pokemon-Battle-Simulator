...
const BattlePage = () => {
    ...
    useEffect(() => {
        // Calculate the total HP of Cynthia's team
        const totalHP = Object.values(cynthiaTeamHP).reduce((acc, hp) => acc + hp, 0);
    
        // Check if the total HP is 0 or less
        if (totalHP <= 0) {
            alert("Cynthia has no more Pokémon left! You win!");
            // Optionally, you can perform additional actions here, such as resetting the game or updating the game state to reflect the end of the battle.
        }
    }, [cynthiaTeamHP]); // Depend on cynthiaTeamHP to re-run this effect when it changes

    useEffect(() => {
        // Calculate the total HP of your team
        const totalHP = Object.values(ourTeamHP).reduce((acc, hp) => acc + hp, 0);
    
        // Check if the total HP is 0 or less
        if (totalHP <= 0) {
            alert("All your Pokémon have fainted! You've lost the battle.");
            // Optionally, you could navigate the player to a different screen or offer options to restart the battle or go to a safe point.
        }
    }, [ourTeamHP]); // Depend on ourTeamHP to re-run this effect when it changes
