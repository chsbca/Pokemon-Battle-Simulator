            ...
            if (user) {
                ...
                try {
                    ...
                    const teamWithMoves = await Promise.all(response.data.team.map(async (teamMember) => {
                        try {
                            console.log("Fetching moves for:", teamMember);
                            const movesResponse = await axios.get(`http://localhost:8000/api/team_pokemon/29/chosen_moves`, {
                                headers: { Authorization: `Token ${token}` }
                            });
                            return {
                                ...teamMember,
                                learnedMoves: movesResponse.data.length > 0 ? movesResponse.data : [{ name: 'No moves learned!' }]
                            };
                        } catch (error) {
                            console.error('Error fetching moves for:', teamMember, error);
                            return {
                                ...teamMember,
                                learnedMoves: [{ name: 'Failed to fetch moves' }]
                            };
                        }
                    }));
