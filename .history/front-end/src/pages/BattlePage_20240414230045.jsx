...
const BattlePage = () => {
    ...
    useEffect(() => {
        const fetchTeams = async () => {
            try {
                // Fetching the team of the logged-in user
                if (user && user.email) {
                    const responseOurTeam = await axios.get(`/api/user_team/${encodeURIComponent(user.email)}`);
                    setOurTeam(responseOurTeam.data);
                }

                // Fetching the team of the opponent
        ...
        fetchTeams();
    }, [opponentEmail, user]); // Add user as a dependency to react to changes
