...
//             })));
//             console.log("Learnable moves set:", learnableMoves);
...
//     };
    ...
    const fetchLearnableMoves = async (pokemonId) => {
        try {
            const token = localStorage.getItem('token');
            const response = await axios.get(`http://localhost:8000/api/pokemon/${pokemonId}/moves/`, {
                headers: { Authorization: `Token ${token}` }
            });
            console.log('Fetched learnable moves:', response.data);  // Ensure this is an array of moves
    ...
    const addChosenMove = async (moveId, teamPokemonId) => {
        const move = learnableMoves.find(m => m.id === moveId); // Find the move in the learnableMoves array
        if (!move) {
            console.error('Move not found in learnableMoves:', moveId);
            return;
        }
        ...
        try {
            ...
            // Update the team state to reflect the new move
            setTeam(prevTeam => prevTeam.map(pokemon => {
                if (pokemon.id === teamPokemonId) {
                    return {...pokemon, learnedMoves: [...pokemon.learnedMoves, {id: move.id, name: move.name}]};
                }
                return pokemon;
            }));
        } catch (error) {
