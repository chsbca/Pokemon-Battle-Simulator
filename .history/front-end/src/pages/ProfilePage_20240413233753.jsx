...
//                                     {pokemon.chosenMoves && pokemon.chosenMoves.length > 0 ? (
//                                         <ul>
//                                             {pokemon.chosenMoves.map((move, idx) => <li key={idx}>{move.name}</li>)}
//                                         </ul>
//                                     ) : (
...
export default function ProfilePage() {
    ...
    const fetchLearnableMoves = async (pokemonId, pokemonName, teamPokemonId) => {
        try {
            const token = localStorage.getItem('token');
            const response = await axios.get(`http://localhost:8000/api/pokemon/${pokemonId}/moves`, {
                headers: { Authorization: `Token ${token}` }
            ...
            setSelectedPokemon(pokemonName); // Set the selected Pokémon's name for the offcanvas header
            setShowOffcanvas(true); // Open the offcanvas to show the moves
            setSelectedTeamPokemonId(teamPokemonId); // Set the ID of the team Pokémon
        } catch (error) {
            console.error('Failed to fetch learnable moves:', error);
            setLearnableMoves([]); // Reset to empty if there's an error
        }
                ...
                <Offcanvas.Body>
                    {learnableMoves.length ? (
                        learnableMoves.map((move, idx) => (
                            <p key={idx}>
                                <button onClick={() => learnMove(move.id, selectedTeamPokemonId)} className="btn btn-primary" style={{ margin: '5px' }}>
                                    Learn Move
                                </button>
                                {move.name}
                            </p>
