...
export default function ProfilePage() {
        ...
        const fetchTeam = async () => {
            if (user) {
                ...
                try {
                    ...
                    // Fetch learned moves for each Pokémon in the team
                    await Promise.all(response.data.team.map(async (teamPokemon) => {
                        const movesResponse = await axios.get(`http://localhost:8000/api/team_pokemon/${teamPokemon.id}/learned_moves`, {
                            headers: { Authorization: `Token ${token}` }
                        });
                        return { ...teamPokemon, learnedMoves: movesResponse.data };
                    })).then(teamWithMoves => {
                        setTeam(teamWithMoves); // Update state with learned moves included
                    });
    ...
    const fetchLearnedMoves = async (teamPokemonId) => {
        if (!teamPokemonId) {
            console.error('Invalid teamPokemon ID');
            return;
        }
        try {
            const token = localStorage.getItem('token');
            ...
            console.log("Learned moves fetched for teamPokemon ID " + teamPokemonId + ":", response.data);
