...
const BattlePage = () => {
    ...
    const fetchCynthiasDecision = async () => {
        try {
            const response = await axios.post('/api/cynthia_decision/', {
                current_state: {
                    // Include necessary state details here
            ...
            if (response.data) {
                console.log("Cynthia's decision:", response.data);
                handleCynthiaDecision(response.data);
            }
        } catch (error) {
            console.error('Failed to fetch Cynthia\'s decision:', error);
        }
    };
