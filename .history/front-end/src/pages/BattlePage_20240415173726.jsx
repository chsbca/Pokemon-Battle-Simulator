            ...
            {currentPokemon && cynthiaPokemon ? (
                <>
                    <Row className="justify-content-center">
                        ...
                        <Col md={4}>
                        <Row className="justify-content-center">
    {cynthiaTeam.map((pokemon, index) => (
        <Col md={4} key={index}>
            <Card>
                <Card.Img variant="top" src={pokemon.pokemon.sprite} />
                <Card.Body>
                    <Card.Title>{pokemon.pokemon.name}</Card.Title>
                    <ProgressBar now={(cynthiaTeamHP[pokemon.pokemon.pokedex_number] / pokemon.pokemon.hp) * 100} label={`HP: ${cynthiaTeamHP[pokemon.pokemon.pokedex_number]} / ${pokemon.pokemon.hp}`} />
                    <Card.Text>Types: {pokemon.pokemon.types.map(t => t.name).join(', ')}</Card.Text>
                    <Accordion defaultActiveKey="0">
                        <Accordion.Item eventKey="0">
                            <Accordion.Header>
                                Moves for {pokemon.pokemon.name} - HP: {cynthiaTeamHP[pokemon.pokemon.pokedex_number]} / {pokemon.pokemon.hp}
                            </Accordion.Header>
                            <Accordion.Body>
                                <ul>
                                    {pokemon.chosen_moves.map((move, idx) => (
                                        <li key={idx}>{move.learnable_move.name}</li>
                                    ))}
                                </ul>
                            </Accordion.Body>
