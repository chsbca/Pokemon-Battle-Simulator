...
const BattlePage = () => {
    ...
    const performBattle = (attacker, defender, move, userAttacksFirst) => {
        const calculateDamage = (attacker, defender, move) => {
            ...
            defender.pokemon.types.forEach(defenderType => {
                attacker.pokemon.types.forEach(attackerType => {
                    // Simulating a function to get type effectiveness from a utility or service
        ...
        const userDamage = calculateDamage(attacker, defender, move);
        let newDefenderHP = Math.max(defender.pokemon.hp - userDamage, 0);
    
        // Update HP based on who is attacking
        if (userAttacksFirst) {
            setCynthiaTeamHP(prevHP => ({ ...prevHP, [defender.pokemon.pokedex_number]: newDefenderHP }));
        } else {
            setOurTeamHP(prevHP => ({ ...prevHP, [defender.pokemon.pokedex_number]: newDefenderHP }));
        }
    
        setEvents(prevEvents => [`${attacker.pokemon.name} uses ${move.learnable_move.name} and deals ${userDamage} damage!`, ...prevEvents]);
    
        if (newDefenderHP <= 0) {
            const victoryMessage = `${defender.pokemon.name} fainted!`;
            setEvents(prevEvents => [victoryMessage, ...prevEvents]);
            // Handle pokemon switching or battle end
        } else {
            // If defender survives, they get to attack back if user attacked first
            if (userAttacksFirst) {
                setTimeout(() => performBattle(defender, attacker, selectRandomMove(defender), !userAttacksFirst), 1000);
            }
        }
    };
