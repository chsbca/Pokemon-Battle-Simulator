...
export default function ProfilePage() {
    ...
    const addChosenMove = async (learnableMoveId) => {
        ...
        if (!move) {
            console.error('Move not found in learnableMoves');
            return;
        }
    
        console.log(`Attempting to add move: ${move.name} (ID: ${move.learnableMoveId}) to teamPokemonId: ${selectedPokemon.id}`);
        try {
            const token = localStorage.getItem('token');
            const response = await axios.post(`http://localhost:8000/api/team_pokemon/${selectedPokemon.id}/chosen_moves/`, {
                learnable_move_id: move.learnableMoveId  // Make sure to change this parameter according to your backend expectation
            }, {
                headers: { Authorization: `Token ${token}` }
            });
            console.log(`Move '${move.name}' added successfully:`, response.data);
