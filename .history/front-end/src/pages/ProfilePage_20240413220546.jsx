...
export default function ProfilePage() {
    ...
    const fetchLearnableMoves = async (pokemonId, pokemonName) => {
        try {
            ...
            setLearnableMoves(response.data); // Assume response.data holds the moves
        ...
        <div>
                    ...
                    {team.length > 0 ? team.map((pokemon, index) => (
                        <Accordion.Item eventKey={index.toString()} key={index}>
                            <Accordion.Header>Pokémon {index + 1} - {pokemon.name}
                                <button className="btn btn-info ms-auto" style={{ margin: '0 10px' }} onClick={() => fetchLearnableMoves(pokemon.pokedex_number, pokemon.name)}>
                                    View Moves
                                </button>
                            </Accordion.Header>
                            <Accordion.Body>
            ...
            <Offcanvas.Body>
                {learnableMoves.length ? (
                    learnableMoves.map((move, idx) => <p key={idx}>{move.name}</p>)
                ) : (
