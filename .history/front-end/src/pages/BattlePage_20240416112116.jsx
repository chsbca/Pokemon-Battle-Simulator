    ...
    const fetchCynthiasDecision = async (pokemon) => {
        try {
            const response = await axios.post('http://localhost:8000/api/teams/cynthia_decision/', {
                pokemon: pokemon.pokemon.pokedex_number,  // Assuming you need the Pokedex number
                ourTeam: ourTeam.map(p => p.pokemon.pokedex_number),  // Send relevant identifiers
                ourTeamHP: ourTeamHP,
                cynthiaTeamHP: cynthiaTeamHP
            });
            console.log("Decision received:", response.data);
            return response.data;  // Handle the response data appropriately
        } catch (error) {
