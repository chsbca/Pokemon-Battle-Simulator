...
//     //                 console.log("Learnable moves set:", mappedMoves);
//     //             } else {
//     //                 setLearnableMoves([]);
//     //             }
...
//     //         console.log(`Attempting to add move: ${move.name} (ID: ${move.id}) to teamPokemonId: ${teamPokemonId}`);
//     //         if (!moveId) {
...
//             // Re-fetch and log the learned moves to see the update immediately
...
//                                     onClick={() => {
//                                         console.log(`Fetching learnable moves for Pokémon ID: ${pokemon.pokedex_number}`);
//                                         fetchLearnableMoves(pokemon.pokedex_number, pokemon.name)
//                                     }}>
...
export default function ProfilePage() {
    ...
    const fetchTeam = async () => {
        ...
        try {
            ...
            const teamWithMoves = await Promise.all(response.data.team.map(async (teamMember) => {
                ...
                return { ...teamMember, learnedMoves: movesResponse.data };
            }));
            setTeam(teamWithMoves);
        } catch (error) {
            console.error('Failed to fetch team:', error);
    ...
    const addToTeam = async (pokemon) => {
        ...
        const token = localStorage.getItem('token');
        try {
            const response = await axios.post('http://localhost:8000/api/user_team/', { pokemon_id: pokemon.pokedexNumber }, { headers: { Authorization: `Token ${token}` } });
            setTeamUpdated(prev => !prev); // Trigger re-fetch of the team
        } catch (error) {
    ...
    const removeFromTeam = async (pokemonId) => {
        const token = localStorage.getItem('token');
        try {
            await axios.delete(`http://localhost:8000/api/user_team/${pokemonId}/`, { headers: { Authorization: `Token ${token}` } });
    ...
    const fetchLearnableMoves = async (teamPokemonId, pokedexNumber, pokemonName) => {
        const token = localStorage.getItem('token');
        try {
            const response = await axios.get(`http://localhost:8000/api/pokemon/${pokedexNumber}/moves/`, { headers: { Authorization: `Token ${token}` } });
            setLearnableMoves(response.data.map(move => ({ id: move.id, name: move.name, description: move.description })));
            setSelectedPokemon({ id: teamPokemonId, pokedexNumber: pokedexNumber, name: pokemonName });
            setShowOffcanvas(true);
        } catch (error) {
            console.error('Failed to fetch learnable moves:', error);
    ...
    const addChosenMove = async (moveId) => {
        ...
        const token = localStorage.getItem('token');
        try {
            const response = await axios.post(`http://localhost:8000/api/team_pokemon/${selectedPokemon.id}/chosen_moves/`, { move_id: moveId }, { headers: { Authorization: `Token ${token}` } });
            fetchChosenMoves(selectedPokemon.id); // Re-fetch learned moves
        } catch (error) {
            console.error(`Error adding chosen move '${move?.name}':`, error);
        }
    };
    ...
    const fetchChosenMoves = async (teamPokemonId) => {
        const token = localStorage.getItem('token');
        try {
            const response = await axios.get(`http://localhost:8000/api/team_pokemon/${teamPokemonId}/chosen_moves/`, { headers: { Authorization: `Token ${token}` } });
