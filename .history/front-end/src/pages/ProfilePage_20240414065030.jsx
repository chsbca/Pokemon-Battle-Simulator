...
export default function ProfilePage() {
    ...
    const addChosenMove = async (learnableMoveId) => {
        const move = learnableMoves.find(m => m.id === learnableMoveId);
        if (!move) {
            console.error('Move not found');
            return;
        }
    
        console.log(`Adding move: ${move.name} (LearnableMove ID: ${learnableMoveId}) to TeamPokemon ID: ${selectedPokemon.id}`);
        try {
            const token = localStorage.getItem('token');
            const response = await axios.post(`http://localhost:8000/api/team_pokemon/${selectedPokemon.id}/chosen_moves/`, {
                learnable_move_id: learnableMoveId  // Use the learnableMoveId directly
            }, {
                headers: { Authorization: `Token ${token}` }
            });
            console.log(`Move '${move.name}' added successfully:`, response.data);
            // Optionally re-fetch the chosen moves to update the UI
        } catch (error) {
            console.error(`Error adding chosen move '${move.name}':`, error.response ? error.response.data : error);
        }
    };
