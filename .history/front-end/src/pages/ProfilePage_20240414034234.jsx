...
// Assuming fetchLearnableMoves has been updated to use the new API response structure

    const fetchLearnableMoves = async (pokemonId, pokemonName) => {
        setIsLoading(true);  // Set a loading state
        try {
            const response = await axios.get(`http://your-api/moves/${pokemonId}`);
            setLearnableMoves(response.data);  // Assuming data includes ids now
