...
import { Container, Row, Col, Card, Button, ProgressBar, Tabs, Tab, Accordion, Toast, ToastContainer } from 'react-bootstrap';
...
const BattlePage = () => {
    ...
    const executeAttack = async (attacker, defender, move) => {
        ...
        const newEvent = {
            ...
            message: `${attacker.pokemon.name} uses ${move.learnable_move.name} and deals ${damage} damage! ${effectivenessMessage}`,
            show: true // Initially set to show the toast
        };
    ...
    const createBattleEvent = async (attacker, move, damage, effectivenessMessage) => {
        ...
        try {
            ...
            setEvents(prevEvents => [response.data.commentary, ...prevEvents]);
        } catch (error) {
    ...
    const handleHide = (id) => {
        setEvents(events => events.map(event => 
            event.id === id ? { ...event, show: false } : event
        ));
    };
    ...
    <ToastContainer position="top-end">
        {events.map((event) => (
            <Toast key={event.id} onClose={() => handleHide(event.id)} show={event.show} delay={3000} autohide>
                <Toast.Header>
                    <strong className="me-auto">Battle Event</strong>
                    <small>just now</small>
                </Toast.Header>
                <Toast.Body>{event.message}</Toast.Body>
            </Toast>
        ))}
    </ToastContainer>
</Col>
