...
import { Accordion, Offcanvas, Container } from 'react-bootstrap';
...
//                 <img src="/profile.png" alt="Porofile" style={{ maxWidth: '30%', height: 'auto' }} />
//             </div>
...
//                                                     x
//                                                 </button>
//                                                 {capitalizeAndFormat(move.name)}</div>
//                                         ))
//                                     ) : (
...
return (
    <Container>
      <Row>
        <Col md={8}> {/* First Column for Pokémon Team */}
          <h3>Your Pokémon Team:</h3>
          {isLoading ? <p>Loading...</p> :
            <Accordion>
              {team.length > 0 ? team.map((pokemon, index) => (
                <Accordion.Item eventKey={index.toString()} key={index}>
                  ...
                  <Accordion.Body>
                    {/* All your buttons and info for Pokémon */}
                  </Accordion.Body>
                </Accordion.Item>
              )) : <p>No Pokémon assigned to your team.</p>}
            </Accordion>
        ...
        <Col md={4}> {/* Second Column for Searching and Search Results */}
          <h3>Search for Pokémon:</h3>
          <input
            type="text"
            placeholder="Search Pokémon..."
            value={searchTerm}
            ...
            className="mb-2"
          />
          <Button onClick={handleSearch} className="me-2">Search</Button>
          <Button onClick={clearSearchResults} variant="secondary">Clear</Button>
            ...
            <>
              ...
              <div>
                {searchResults.map((pokemon, index) => (
                  <div key={index}>
                    {pokemon.name}
                    <Button onClick={() => addToTeam(pokemon)} className="ms-2">Add to Team</Button>
                  </div>
                ))}
              </div>
            </>
