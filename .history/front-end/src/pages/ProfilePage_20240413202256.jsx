...
export default function ProfilePage() {
    ...
    const addToTeam = async (pokemon) => {
        try {
            const token = localStorage.getItem('token');
            const response = await axios.post('http://localhost:8000/api/user_team/', { pokemon_id: pokemon.id }, {
                headers: { Authorization: `Token ${token}` }
            });
            console.log(response.data.message); // Optionally, refresh team list or show a message
        } catch (error) {
