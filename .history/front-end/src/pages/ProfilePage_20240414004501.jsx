        ...
        const fetchTeamAndMoves = async () => {
            if (user) {
                ...
                try {
                    ...
                    const teamWithMoves = await Promise.all(response.data.team.map(async (pokemon) => {
                        const movesResponse = await axios.get(`http://localhost:8000/api/team_pokemon/${pokemon.id}/learned_moves`, {
                            headers: { Authorization: `Bearer ${token}` },
                        });
                        return {...pokemon, learnedMoves: movesResponse.data};
                    }));
                    setTeam(teamWithMoves);
                } catch (error) {
                    console.error('Failed to fetch team and moves:', error);
                } finally {
                    setIsLoading(false);
                }
            }
