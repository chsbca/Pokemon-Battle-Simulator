...
const BattlePage = () => {
    ...
    useEffect(() => {
        const fetchTeams = async () => {
            try {
                // Assuming you have a way to get the logged-in user's email or use auth
                const ourEmail = 'ourUser@example.com'; // Replace with actual method to get email or auth token
                const responseOurTeam = await axios.get(`/api/user_team/${encodeURIComponent(ourEmail)}`);
                const responseOpponentTeam = await axios.get(`/api/user_team_by_email/${encodeURIComponent(opponentEmail)}`);
                setOurTeam(responseOurTeam.data);
                setOpponentTeam(responseOpponentTeam.data);
            } catch (error) {
    ...
    return (
        <Container className="mt-5 text-center">
            ...
            <Row className="justify-content-between align-items-start">
                <Col md={4}>
                    {/* Example card for our Pokémon, adjust indexing as needed */}
                    {ourTeam.length > 0 && (
                        <Card>
                            <Card.Img variant="top" src="path_to_sprite" /> {/* Update this path */}
                            <Card.Body>
                                <Card.Title>{ourTeam[0].pokemon_name}</Card.Title>
                                {/* Progress bars and other data */}
                            </Card.Body>
                        </Card>
                    )}
                </Col>
                <Col md={4}>
                    {/* Event log or other middle column content */}
                </Col>
                <Col md={4}>
                    {/* Example card for opponent's Pokémon */}
                    {opponentTeam.length > 0 && (
                        <Card>
                            <Card.Img variant="top" src="path_to_sprite" /> {/* Update this path */}
                            <Card.Body>
                                <Card.Title>{opponentTeam[0].pokemon_name}</Card.Title>
                                {/* Progress bars and other data */}
                            </Card.Body>
                        </Card>
                    )}
