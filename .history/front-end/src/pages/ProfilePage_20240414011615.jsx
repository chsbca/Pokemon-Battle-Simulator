...
export default function ProfilePage() {
        ...
        const fetchTeam = async () => {
            if (user) {
                ...
                try {
                    ...
                    console.log("Team data fetched:", response.data.team);
    
                    const teamWithMoves = await Promise.all(response.data.team.map(async (teamMember) => {
                        console.log("Attempting to fetch moves for:", teamMember);
                        if (!teamMember.teamMemberId) {
                            console.error("No identifier found for team member:", teamMember);
                            return teamMember; // Return the member without moves if no ID is found
                        }
                        try {
                            const movesResponse = await axios.get(`http://localhost:8000/api/team_pokemon/${teamMember.teamMemberId}/learned_moves`, {
                                headers: { Authorization: `Token ${token}` }
                            });
                            return { ...teamMember, learnedMoves: movesResponse.data };
                        } catch (error) {
                            console.error('Failed to fetch learned moves for:', teamMember, error);
                            return { ...teamMember, learnedMoves: [{ name: 'Failed to fetch moves' }] };
                        }
                    }));
    ...
    const addChosenMove = async (teamPokemonId, moveId) => {
        try {
            ...
            fetchChosenMoves(teamPokemonId); // Refresh the list of chosen moves
        } catch (error) {
            console.error('Error adding move:', error.response ? error.response.data : error);
        }
    };
