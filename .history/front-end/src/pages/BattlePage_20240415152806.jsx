...
const BattlePage = () => {
    ...
    function calculateDamage(attacker, defender, move, level = 50) {
        // Base power of the move
        const basePower = move.power;
    
        // Determine if the move is physical or special
        const isPhysical = move.move_type.category === 'physical'; // Assume move_type includes a category property
    
        // Choose the appropriate stats based on the type of move
        //update this to reflect phys or spec later
        const attackStat = attacker.stats.attack > attacker.stats.special_attack ? attacker.stats.attack : attacker.stats.special_attack;
        const defenseStat = defender.stats.defense > defender.stats.special_defense ? defender.stats.defense : defender.stats.special_defense;
    
        // STAB calculation
        const stab = attacker.types.includes(move.move_type.name) ? 1.5 : 1;
    
        // Type Effectiveness
        let typeEffectiveness = 1;
        defender.types.forEach(defType => {
            const effectiveness = move.move_type.effectiveness_relations.find(eff => eff.target_type === defType.name);
            if (effectiveness) {
                switch(effectiveness.effectiveness) {
                    ...
                    case 'not_effective':
                        ...
                        break;
                    default:
                        typeEffectiveness *= 1; // Normal effectiveness
                }
            }
        });
    
        // Damage formula
        const damage = (((2 * level / 5 + 2) * basePower * attackStat / defenseStat) / 50 + 2) * stab * typeEffectiveness;
        return Math.floor(damage); // Return an integer value for damage
    }
    
    // Example usage: Assume we have 'attacker', 'defender', and 'move' objects populated appropriately
    // console.log(calculateDamage(attacker, defender, move));
