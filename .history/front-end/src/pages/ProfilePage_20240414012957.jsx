...
export default function ProfilePage() {
            ...
            if (user) {
                ...
                try {
                    ...
                    const teamWithMoves = await Promise.all(response.data.team.map(async (teamMember) => {
                        console.log("Team Member ID:", teamMember.id);  // Ensure this logs a valid identifier
                        if (!teamMember.id) {
                        ...
                        try {
                            ...
                            console.error('Error fetching moves for:', teamMember, error);
    ...
    const fetchLearnableMoves = async (pokemonId, pokemonName) => {
        try {
            ...
            setSelectedPokemon(pokemonId); // Set the selected Pokémon's name for the offcanvas header
                ...
                <Accordion>
                    {team.length > 0 ? team.map((pokemon, index) => (
                        <Accordion.Item eventKey={index.toString()} key={index}>
                            ...
                            <Accordion.Body>
                                <button className="btn btn-info mb-3" onClick={() => fetchLearnableMoves(pokemon.pokedex_number, pokemon.id)}>
            ...
            <Offcanvas show={showOffcanvas} onHide={() => setShowOffcanvas(false)} placement="end">
                ...
                <Offcanvas.Body>
                    {learnableMoves.length ? (
                        learnableMoves.map((move, idx) => <p key={idx}><button onClick={() => addChosenMove(move.id, selectedPokemon)} className="btn btn-primary" style={{ margin: '5px' }}>
                            +
                        </button>{move.name}
                        </p>)
                    ) : (
