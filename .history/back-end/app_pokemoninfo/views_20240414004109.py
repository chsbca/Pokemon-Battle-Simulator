...
@api_view(['GET'])
def get_learned_moves(request, team_pokemon_id):
    try:
        team_pokemon = TeamPokemon.objects.get(id=team_pokemon_id)
        learned_moves = ChosenMoves.objects.filter(team_pokemon=team_pokemon).select_related('learnable_move', 'learnable_move__move')
        moves_data = [{'id': move.id, 'name': move.learnable_move.move.name} for move in learned_moves]
        return Response(moves_data)
    except TeamPokemon.DoesNotExist:
