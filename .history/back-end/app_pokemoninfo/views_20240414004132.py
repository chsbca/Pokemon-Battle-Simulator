...
class ChosenMovesView(APIView):
    def get(self, request, team_pokemon_id):
        chosen_moves = ChosenMoves.objects.filter(team_pokemon_id=team_pokemon_id)
        moves_data = [{'move_name': move.learnable_move.move_name} for move in chosen_moves]
        return Response(moves_data)

    def post(self, request, team_pokemon_id):
        move_id = request.data.get('move_id')
        try:
            chosen_move = ChosenMoves.objects.create(
                team_pokemon_id=team_pokemon_id,
                learnable_move_id=move_id
            )
            return Response({'message': 'Move added successfully'}, status=201)
        except Exception as e:
