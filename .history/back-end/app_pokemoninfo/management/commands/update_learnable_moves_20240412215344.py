...
class Command(BaseCommand):
    ...
    def update_learnable_moves(self):
        pokemons = Pokemon.objects.all()
        start_from_rotom = False  # Initialize the flag
        for pokemon in pokemons:
            if pokemon.name.lower() == 'rotom' or start_from_rotom:
                start_from_rotom = True  # Set the flag to start updating from Rotom
                response = requests.get(f'https://pokeapi.co/api/v2/pokemon/{pokemon.name.lower()}')
                if response.status_code == 200:
                    pokemon_data = response.json()
                    self.assign_learnable_moves(pokemon, pokemon_data['moves'])
            else:
                self.stdout.write(self.style.NOTICE(f'Skipping {pokemon.name} until reaching Rotom'))
