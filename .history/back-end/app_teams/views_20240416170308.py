...
def create_prompt(battle_details):
    return f"{battle_details['attacker']} uses {battle_details['move']}! It deals {battle_details['damage']} damage and is {battle_details['effectiveness']}! REACT TO THE POKEMON BATTLE WITH EXPLOSIVE, HYPED UP COMMENTARY! Include a pun too! But keep your commentary to like 3-4 sentences only. Also change your reaction depending on if it's super effective, not effective/doesnt affect target, or just no effectiveness application. CHoose one depending on the first sentence in the prompt"
