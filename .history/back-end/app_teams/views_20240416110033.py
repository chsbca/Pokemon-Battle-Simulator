...
def cynthia_decision(request):
    if request.method == 'POST':
        current_state = request.json.get('current_state', {})
        decision = get_cynthias_move(current_state)
        if decision:
            return JsonResponse(decision, safe=False)
        else:
            return JsonResponse({'error': 'Unable to generate move'}, status=500)