...
class CynthiaDecisionAPI(APIView):
    def post(self, request, *args, **kwargs):
        try:
            ...
            # Call OpenAI API to generate a move decision
            response = openai.Completion.create(
              engine="davinci",
              prompt=self.create_prompt(data),  # Transform the input data into a prompt
              max_tokens=150
            )

            decision = {
                'move': response.choices[0].text.strip(),  # Parse the move from the response
                'dialogue': "Cynthia says: " + response.choices[0].text.strip()  # Example dialogue
            }
    ...
    def create_prompt(self, data):
        # Create a natural language prompt from the game state
        prompt = f"Given the current game state {data}, what should Cynthia do next?"
        return prompt
