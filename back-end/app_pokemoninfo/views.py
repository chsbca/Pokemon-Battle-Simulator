...
from .models import Pokemon, LearnableMoves
...
from .serializers import PokemonBattleSerializer, TeamBattleSerializer
...
def get_pokemon_moves(request, pokedex_number):
    pokemon = Pokemon.objects.get(pokedex_number=pokedex_number)
    learnable_moves = LearnableMoves.objects.filter(pokemon=pokemon)
    serialized_moves = MoveSerializer(learnable_moves, many=True)
    return JsonResponse(serialized_moves.data, safe=False)
...
class ChosenMovesView(APIView):
    ...
    def post(self, request, team_pokemon_id):
        ...
        try:
            team_pokemon = TeamPokemon.objects.get(id=team_pokemon_id)
            learnable_move = LearnableMoves.objects.get(move__id=move_id, pokemon=team_pokemon.pokemon)
    ...
    def delete(self, request, team_pokemon_id, move_id=None):
        if not move_id:
            return Response({'error': 'Move ID is required'}, status=status.HTTP_400_BAD_REQUEST)
        try:
            team_pokemon = TeamPokemon.objects.get(id=team_pokemon_id)
            move_to_remove = ChosenMoves.objects.get(team_pokemon=team_pokemon, id=move_id)
            move_to_remove.delete()
            return Response({'message': 'Move removed successfully'}, status=status.HTTP_204_NO_CONTENT)
        except TeamPokemon.DoesNotExist:
            return Response({'error': 'TeamPokemon not found'}, status=status.HTTP_404_NOT_FOUND)
        except ChosenMoves.DoesNotExist:
            return Response({'error': 'Move not found or not valid for this Pokémon'}, status=status.HTTP_404_NOT_FOUND)
        except Exception as e:
            return Response({'error': str(e)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
class PokemonBattleView(APIView):
    ...
    def get(self, request, pokedex_number):
        pokemon = get_object_or_404(Pokemon, pokedex_number=pokedex_number)
        serializer = PokemonBattleSerializer(pokemon)
        return Response(serializer.data)